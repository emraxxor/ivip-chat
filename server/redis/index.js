...
const logger = require('simple-node-logger').createLogManager().createLogger('[REDIS]')
...
Redis.prototype.addUser = function (room, uid, udata) {
  logger.info(`[REDIS] Add ${uid} to ${room} udata: ${JSON.stringify(udata)}`)
...
Redis.prototype.users = function () {
  return this.client
    .hgetallAsync('users')
    .then(users => {
      if (users) { return Object.keys(users).map(e => JSON.parse(users[e])) }
...
Redis.prototype.getUserByRoom = function (room, uid) {
  return this.client
    ...
    .then(
      res => JSON.parse(res),
      err => { logger.info('getUserByRoom ', err) }
    )
}
...
Redis.prototype.setPrivateChat = async function (ufrom, data) {
  return await this.client
    ...
    .then(
      ...
      err => { logger.info('setPrivateChat ', err) }
    )
}
...
Redis.prototype.removeCamera = async function (ufrom, uto) {
  let cams = await this.getCamera(ufrom)
  if (cams && cams.indexOf(uto) !== -1) {
...
Redis.prototype.getPrivateChat = async function (ufrom) {
  return await this.client
    ...
    .then(
      res => JSON.parse(res),
      err => { logger.info('getPrivateChat ', err) }
    )
}
...
Redis.prototype.deleteUserFromPrivates = function (username) {
  this.client
    ...
    .then(
      res => (res),
      err => { logger.info('deleteUserFromPrivates ', err) }
    )
}
...
Redis.prototype.deleteUserFromRoom = function (room, uid) {
  ...
  if (!this.getUserByRoom(room, uid)) { return }

  this.client
    .hdelAsync(room, uid)
    .then(
      res => (res),
      err => { logger.info('deleteUserFromRoom ', err) }
    )
}
...
Redis.prototype.deleteUser = function (uid) {
  this.client
    ...
    .then(
      res => (res),
      err => { logger.info('deleteUser ', err) }
    )
}
